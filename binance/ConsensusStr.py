...
class ConsensusStr(IStrategy):
    ...
    buy_score_long = IntParameter(low=0, high=100, default=45, space='buy', optimize=buy_optimize)
    ...
    sell_score_long = IntParameter(low=0, high=100, default=88, space='sell', optimize=sell_optimize)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                    (dataframe['consensus_buy'] > self.buy_score_long.value) &
                    (dataframe['volume'] > 0)
            ),
            'enter_long'] = 1
        return dataframe
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                    (dataframe['consensus_sell'] > self.sell_score_long.value) &
                    (dataframe['volume'] > 0)
            ),
            'exit_long'] = 1
